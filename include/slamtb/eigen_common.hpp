...
inline STB_DEVICE_HOST Eigen::Matrix<scalar_t, 2, 1> to_vec2(
    const Accessor acc) {
  return Eigen::Matrix<scalar_t, 2, 1>(acc[0], acc[1], acc[2]);
}

template <typename scalar_t, typename Accessor>
inline STB_DEVICE_HOST Eigen::Matrix<scalar_t, 3, 1> to_vec3(
    const Accessor acc) {
  return Eigen::Matrix<scalar_t, 3, 1>(acc[0], acc[1], acc[2]);
}

template <typename scalar_t, typename Accessor>
inline STB_DEVICE_HOST Eigen::Matrix<scalar_t, 4, 1> to_vec4(
    const Accessor acc) {
  return Eigen::Matrix<scalar_t, 4, 1>(acc[0], acc[1], acc[2], acc[3]);
}

template <typename scalar_t, int rows, int cols, typename Accessor>
inline STB_DEVICE_HOST Eigen::Matrix<scalar_t, rows, cols> to_matrix(
    const Accessor acc) {
