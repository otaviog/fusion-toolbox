...
#pragma nv_exec_check_disable
  FTB_DEVICE_HOST void ComputeJacobian(int ri,
  ...
  FTB_DEVICE_HOST void operator()(int ri) {
    for (int k = 0; k < 6; k++) Jtr_partial[ri][k] = 0;
    ...
    ComputeJacobian(ri, Tsrc_point, u, v, jacobian, residual);

    for (int k = 0; k < 6; ++k) {
      Jtr_partial[ri][k] = jacobian[k] * residual;
    }

    for (int krow = 0; krow < 6; ++krow) {
      for (int kcol = krow; kcol < 6; ++kcol) {
        const scalar_t feat_v = jacobian[kcol] * jacobian[krow];
        JtJ_partial[ri][krow][kcol] = feat_v;
        JtJ_partial[ri][kcol][krow] = feat_v;
      }
    }

    squared_residual[ri] = residual;
  }
};
  ...
  if (reference_dev.is_cuda()) {
    AT_DISPATCH_FLOATING_TYPES(
        src_points.scalar_type(), "EstimateFeature", ([&] {
          ...
          FeatureJacobianKernel<kCUDA, scalar_t> kernel(
    ...
    AT_DISPATCH_FLOATING_TYPES(
        src_points.scalar_type(), "EstimateFeature", [&] {
          ...
          FeatureJacobianKernel<kCPU, scalar_t> kernel(
